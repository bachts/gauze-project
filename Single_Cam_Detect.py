...
while True:
    ...
    # frame4 = frame3
    frame0=frame1
    frame0=cv2.cvtColor(frame1,cv2.COLOR_BGR2RGBA).astype(np.float32)
    frame0=jetson.utils.cudaFromNumpy(frame0)

    detections=net.Detect(frame0, width, height)
    ...
    for detect in detections:
        ...
        cv2.rectangle(frame1, (int(left), int(bottom)), (int(right), int(top)), (0,255,0), 2) #draw rectangle around gauze
        cv2.putText(frame1,'Gauze '+str(confidence)+'%',(int(left),int(top)),font,1,(0,255,255),2) #write "gauze" and confidence above rectangle
    ...
    borderedFrame = cv2.copyMakeBorder(frame1,0,350,0,0,cv2.BORDER_CONSTANT,value=[0,0,0])
