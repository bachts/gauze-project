...
# CAM11 is for video purposes only. 
# Red lined is to fix the confidence percentage on the screen for easier making of video.
# NMS set at 0.9 for detectNet.h file
...
net=jetson.inference.detectNet(argv=['--model=models/gauzecombine1/ssd-mobilenet.onnx', '--labels=models/gauzecombine1/labels.txt', '--input-blob=input_0', '--output-cvg=scores', '--output-bbox=boxes', '--threshold=0.6'])
...
while True:
    ...
    for detect in detections:
        ...
        FAKEconfidence = str(random.randint(96,99)) + "." + str(random.randint(0,9))
